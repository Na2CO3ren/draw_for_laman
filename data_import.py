        ...
        for index, line in enumerate(lines[1:]):
            ...
            if not point.pointId in substance.cfgs :
                curve = model.Line(point, wave_number, np.array(intensity),
                               ...
                               [])
                substanceList = thd.MatchSubstance(np.array(intensity))
                point.SetColor(substance.cfgs, curve)
                if len(substanceList) > 0:
                    point.components = substanceList
                    colorsForSubstance = [const.SubstanceColorMap[component] for component in substanceList]
                    color = util.MixColors(colorsForSubstance)
                    point.color = color
            else :
                curve = model.Line(point, wave_number, np.array(intensity),
